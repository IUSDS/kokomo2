...
def store_booking_to_db(payload: dict):
    ...
    try:
        ...
        if not member_id:
            raise HTTPException(
                status_code=400, detail="member_id is required in the payload."
            )
        ...
        if not cursor.fetchone():
            raise HTTPException(
                status_code=400,
                detail=f"member_id {member_id} does not exist in Members table.",
            )
...
def parse_booking_payload(
    booking: dict, member_id: int, point_cost: int, booking_fee: float = 0.0
) -> dict:
    ...
    return {
        ...
        "number_of_adults": extract_int(
            get_custom_value("How many people are in your party? (no pricing)", 0)
        ),
        "number_of_kids": extract_int(
            get_custom_value("How many kids in your party are under 6?", 0)
        ),
...
def new_record_in_point_adjustment(
    member_id: int, points_added: int, Balance: int, description: str
):  ### After cancellation points adjustments
    try:
        conn = get_db_connection()
        cursor = conn.cursor()
        cursor.execute(
