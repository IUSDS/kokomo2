...
from utils.booking_util import (
    parse_booking_payload,
    store_booking_to_db,
    if_booking_exists,
    charter_booking_exists,
    determine_source,
    get_points_from_members,
    new_record_in_point_adjustment,
    get_points_cost_and_member_id_from_booking_fh,
)
...
from utils.point_pricing_util import (
    get_point_cost,
    deduct_member_points,
    get_curr_points,
)
...
async def webhook_listener(request: Request):
    ...
    print(
        f"INFO: Availability: yacht_name={yacht_name}, tour_type={tour_type_name}, start_at={start_at}, end_at={end_at}"
    )
    ...
    # THIS HAS TO BE DONE ONLY IF THE SOURCE OF THE WEBHOOK IS KYC
    # THIS WHOLE THING HAS TO GO TO LINE 106
    ...
    if if_booking_exists(booking_pk):
        ...
        if status == "cancelled":
            ...
            # THE CURRENT POINTS OF THE MEMBER ARE NOT BEING ADJUSTED IN THE MEMBERS TABLE
    ...
    if webhook_source.upper() == "CHARTERS":
        ...
        if charter_booking_exists(booking_pk):
            ...
            raise HTTPException(
                status_code=409, detail="Charter booking already exists"
            )
